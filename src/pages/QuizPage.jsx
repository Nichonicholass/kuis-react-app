...
function QuizPage() {
  ...
  useEffect(() => {
    ...
    // Pastikan user.username ada
    const savedState = getGameState(user.username);

    if (savedState) {
      console.log("Resume Game for : ", user.username);
      ...
      console.log("New Game Category:", categoryId);
      const loadData = async () => {
        try {
          // Delay sedikit biar skeleton kelihatan (UX: biar ga kedip cepat)
  ...
  const handleFinish = (lastCorrect = null) => {
    // FIX PENTING: Tambahkan user.username saat clear!
  ...
  // --- RENDER LOADING (Sekarang Pakai Component) ---
